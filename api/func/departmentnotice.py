...
def adddata(num, title, date, link):
    global data
    tmp = [num, title, date, link]
    data.append(tmp)
...
def cba(soup, callsign):
    ...
    while keyword is not None:
        ...
        if callsign == "url":
            ...
            soup = callreq('', url)
        title = soup.find('h1', id="bo_v_title").text.strip()
        ...
        adddata(number, title, datetext, link)
...
def biz(soup):
    ...
    while keyword is not None:
        ...
        adddata(number, title, date, link)
...
def account(soup):
    ...
    while keyword is not None:
        ...
        adddata(number, title, datetext, url)
...
def architecture(soup):
    ...
    while keyword is not None:
        ...
        adddata(number, title, date.text, link)
...
def edu(soup):
    ...
    for i in range(0, len(arr) - 2):
        ...
        adddata(number[i], title, datetext, link)
