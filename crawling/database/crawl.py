...
from database.cafeteria import diet
...
def save_db(table, data_list):
    ...
    if table == "department":
        ...
        sql = f"insert into schedule values (%s, %s, %s, %s)"
    elif table == "dietdormitory":
        sql = f"insert ignore into dietdormitory values (%s, %s, %s, %s, %s, %s)"
    ...
    for row in data:
        try:
            ...
            if row[3] == 'account':
                ...
                notice_data = notice.aed(row[0], notice.callreq(row[1], ''))
            elif row[3] == 'archi':
            ...
            if not notice_data:
                raise ZeroDataError()
            save_db("department", notice_data)
...
def crawling_schedule():
    save_db("schedule", schedule.aca_schedule())

def crawling_diet():
    save_db("dietdormitory", diet.dormcafeteria(notice.callreq('', 'http://knudorm.kangwon.ac.kr/home/sub02/sub02_05_bj.jsp')))
